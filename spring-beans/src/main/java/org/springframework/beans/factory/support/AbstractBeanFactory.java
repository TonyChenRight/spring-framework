...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			 ...
			 * 3、bean的实例化
			 */
			//返回对应的实例，有时候存在诸如 BeanFactory的情况并不是直接返回实例本身而是返回指定方法返回的实例
			...
			//如果beanDefinitionMap中也就是在所有已知加载类中不包括beanName则尝试从 parentBeanFactory中检测
			...
			try {
				 ...
				 */
				//将存储XML配置文件的GenericBeanDefinition 转换为RootBeanDefinition，
	...
	protected Object getObjectForBeanInstance(
		...
		/**
		 * 1、 对FacotryBean 正确性的验证
		 */
		// Don't let calling code try to dereference the factory if the bean isn't a factory.
		//如果指定的name 是 工厂相关（以 & 为前缀）且beanInstance又不是FactoryBean类型则验证不通过
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {
			if (mbd.isSingleton()) {
				/*
				 * 单例模式下注册需要销毁的bean,此方法中会处理实现 DisposableBean的bean
				 * 并且对所有的bean 使用 DistructionAwareBeanPostProcessors处理
